...
const TAG_STYLE = {
  ...
  餐廳: 'bg-orange-100 text-orange-700',
  咖啡: 'bg-stone-100 text-stone-600',
  購物: 'bg-pink-100 text-pink-700',
  溫泉: 'bg-sky-100 text-sky-700',
        ...
        {['全部', ...SPOT_TAGS].map(tag => (
          <button
            ...
            className={`flex-shrink-0 text-xs px-3.5 py-1.5 rounded-full font-medium transition-colors ${
              filterTag === tag ? 'bg-nagoya-red text-white' : 'bg-white text-stone-500 border border-stone-100'
            }`}
          >
            {tag}
          </button>
            ...
            {filtered.map(spot => (
              <div key={spot.id} className="bg-white rounded-2xl p-3.5 shadow-sm border border-stone-100 relative flex flex-col">
                <button
                  ...
                  className="absolute top-2.5 right-2.5 w-5 h-5 flex items-center justify-center text-stone-200 hover:text-red-400 active:scale-90 transition-transform"
                >
                  <X size={14} />
                </button>
                ...
                {spot.mapUrl && (
                  <a
                    ...
                    className="inline-flex items-center gap-1 text-[11px] text-matcha mt-2.5 font-medium"
                  >
                    <MapPin size={10} />地圖
                    <ExternalLink size={9} />
                  </a>
        ...
        <div className="fixed inset-0 z-50 flex flex-col justify-end md:justify-center md:items-center">
          ...
          <div className="relative bg-white rounded-t-3xl md:rounded-3xl md:w-full md:max-w-md px-5 pt-5 pb-8 max-h-[85vh] overflow-y-auto">
                    ...
                    <button
                      ...
                      className={`text-xs px-3 py-1.5 rounded-full border font-medium transition-colors ${
                        form.tags.includes(tag)
                          ? 'bg-nagoya-red text-white border-nagoya-red'
            ...
            <button
              ...
              className="w-full mt-5 bg-nagoya-red text-white py-3 rounded-xl font-semibold text-sm disabled:opacity-40 active:scale-95 transition-transform"
            >
              儲存
            </button>
          </div>
